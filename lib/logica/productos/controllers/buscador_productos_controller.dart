...
class BuscadorProductosController extends GetxController {
  ...
  Future<void> obtenerProductos(String nombreOcodigo) async {
    try {
  ...
  WHERE productos.eliminado = false
    AND (
      productos.nombre ILIKE @busqueda
      OR productos.codigo_de_barras ILIKE @busqueda
    )
...
''');
      ...
      final resp = await Database.conn.execute(
        sql,
        parameters: {'busqueda': '%$nombreOcodigo%'},
      );
